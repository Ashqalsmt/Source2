...
Rep_Vip = (6669024587,6937025378)
Rep_Dev = (5571722913)
rchannel = {"@YamenThon", "@YamenThon1", "@Q_A_VI", "@YamenThon_Gorop", "@Binance_gift1"}
...
async def mybot(): #Code by T.me/T_A_Tl
    ...
    if bot_name.startswith("مسـاعـد"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_message("@BotFather", "يمنثون")
            ...
            await bot.send_message("@BotFather", botname)
            await asyncio.sleep(1)
            await bot.send_file("@BotFather", "yamenthon/baqir/Yemen2.jpg")
            await asyncio.sleep(3)
    ...
    try:
        if BOTLOG:
            Config.ZQ_LOBLOGO = await zq_lo.tgbot.send_file(
                ...
                buttons=[(Button.url("𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉𓅛✓", "https://t.me/YamenThon"),)],
            )
    except Exception as e:
        LOGS.error(e)
        return None
...
async def verifyLoggerGroup():
    ...
    if BOTLOG:
        ...
        photozed = await zq_lo.upload_file(file="yamenthon/baqir/Yemen1.jpg")
        _, groupid = await create_supergroup(
            "مجموعة السجـل يمنثون", zq_lo, Config.TG_BOT_USERNAME, descript, photozed
        )
    ...
    if PM_LOGGER_GROUP_ID != -100:
        ...
        photozed = await zq_lo.upload_file(file="yamenthon/baqir/Yemen.jpg")
        _, groupid = await create_supergroup(
