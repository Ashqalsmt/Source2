    ...
    async for usr in event.client.iter_participants(event.chat_id):
        ...
        if baqir.text:
            ...
            try:
                ...
                return
    BAQIR_BEST_DEV = f"[ᯓ 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة خـاص 🚹](t.me/yamenthon) .\n\n**「❖╎تمت الاذاعـه لـ اعضـاء الكـروب .. بنجـاح  ✅**\n**「❖╎عـدد {success} عضـو**"
    ...
    for user in users:
        ...
        if baqir.text:
            ...
            try:
                ...
                rrr.edit(f"خطأ في إرسال الرسالة إلى {user_entity.id}: {str(e)}")
    BAQIR_BEST_DEV = f"[ᯓ 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉 𝗨𝘀𝗲𝗿𝗯𝗼𝘁 - اذاعـة زاجـل 🕊](t.me/yamenthon) .\n⋆┄─┄─┄─┄┄─┄─┄─┄─┄┄⋆\n**「❖╎تمت الاذاعـه .. بنجـاح  ✅**\n**「❖╎عـدد {success} أشخـاص**"
    await rrr.edit(BAQIR_BEST_DEV, link_preview=False)
