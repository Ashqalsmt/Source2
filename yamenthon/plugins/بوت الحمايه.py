...
@zq_lo.tgbot.on(events.MessageEdited)
async def check_edit_media(event):
    ...
    if event.is_group: #Write Code By T.me/zzzzl1l
        ...
        if not admin and not creator:
            return
    zed_dev = (5571722913,6669024587)  #Write Code By T.me/zzzzl1l
    zelzal = event.sender_id
    malath = zedub.uid
    hhh = event.message.text
    zed_id = event.chat_id
...
# Copyright (C) 2022 Zed-Thon
@zq_lo.tgbot.on(events.ChatAction())
async def _(event):
...
async def _(event):
    if not event.is_private:
        ...
        if not admin and not creator:
            return
    # All Rights Reserved for "Zed-Thon - yamenthon" "زلـزال الهيبـه"
    zed_dev = (5571722913,6669024587)
