...
async def _(zed):
    await edit_or_reply (zed, "𓆰 [𝙎𝙊𝙐𝙍𝘾𝞝 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉 - 𝘿𝙀𝙇𝙀𝙏𝙀](t.me/YamenThon) 🗑♻️𓆪\n**𓍹━─━─━─━─𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉─━─━─━─━𓍻**\n\n **✵│رابـط الحـذف ↬** https://telegram.org/deactivate \n\n\n **✵│بـوت الحـذف  ↬** @LC6BOT ")
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 5571722913:
        return await edit_or_reply(mention, f"**╮ ❐ لك هـذا مطور السورس الاسطوره عاشق الصمت. . قدما حكيت ماقدر اوصفـه 🙊💘 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
...
async def permalink(mention):
    ...
    if not user:
        return
    if user.id == 5571722913:
        return await edit_or_reply(mention, f"**╮ ❐ بلا مفتاله حقك هاذا مطور السورس يا مجنون😒 ❏╰**")
    tag = user.first_name.replace("\u2060", "") if user.first_name else user.username
