...
async def _(event):
    cmd = "ls yamenthon/plugins"
    o = (await _reputils.runcmd(cmd))[0]
    OUTPUT = f"**⌔∮ [𝗦𝗢𝗨𝗥𝗖𝗘 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉](tg://need_update_for_some_feature/) الاضافات:**\n{o}"
...
async def _(event):
    ...
    o = stdout.decode()
    OUTPUT = (
        f"**[𝗦𝗢𝗨𝗥𝗖𝗘 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉](tg://need_update_for_some_feature/) فـارات تـنصيـبك:**\n\n\n{o}"
    )
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
...
async def _(event):
    ...
    stdout, stderr = await process.communicate()
    o = stdout.decode()
    OUTPUT = f"**[𝗦𝗢𝗨𝗥𝗖𝗘 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉](tg://need_update_for_some_feature/) , تم حساب سرعة السيرفر:**\n{o}"
    if len(OUTPUT) > Config.MAX_MESSAGE_SIZE_LIMIT:
        with io.BytesIO(str.encode(OUTPUT)) as out_file:
            out_file.name = "env.text"
            await event.client.send_file(
...
async def zeddd(event):
    ...
    zedt = "**- تاريخ تنصيبـك لـ بـوت يـــمنثون - 𓆩𝙎𝙊𝙐𝙍𝘾𝞝 𝙔𝘼𝙈𝙀𝙉𝙏𝙃𝙊𝙉𓆪**\n\n"
