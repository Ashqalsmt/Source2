...
def get_api_id_and_hash():
    print("yamenthon Session Pyrogram")
    print("\n---------------------------\n")
    try:
...
def telethon_session():
    ...
    # logging in
    try:
        with TelegramClient(StringSession(), API_ID, API_HASH) as yamenthon:
            print("Generating a string session for •yamenthon•")
            try:
                yamenthon.send_message(
                    "me",
                    f"**- هـذا هو كـود سيشـن تيليثـون الخـاص بـك ☑️**:\n\n`{yamenthon.session.save()}`\n\n**- لا تشـارك هـذا الكـود مع اي احـد ⚠️؟!**\n**- فقـط قم بنسخـه ولصقـه في مربعـات التنصيب 🛃**\n\n**- سـورس يـــمنثون : @yamenthon**",
                )
                print(
        ...
        print(er)
        print("If you think It as a Bug, Report to @T_A_Tl\n\n")
...
def pyro_session():
    ...
    with Client(
        name="yamenthon", api_id=API_ID, api_hash=API_HASH, in_memory=True
    ) as pyro:
